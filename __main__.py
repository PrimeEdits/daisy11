...
#               PnO - Discord Bot v3
#
#         Made by Prime, improved by my dicc
#
...
async def _edit_owner(red, owner, no_prompt):
    if owner:
        ...
        print(
            "Remember:\n"
            "ONLY the person who is hosting PnO should be owner."
...
def handle_edit(cli_flags: Namespace):
    ...
    red = Red(cli_flags=cli_flags, description="PnO V3", dm_help=None, fetch_offline_members=True)
    try:
...
def main():
    ...
    try:
        ...
        red = Red(
            cli_flags=cli_flags, description="PnO V3", dm_help=None, fetch_offline_members=True
        )
        ...
        log.warning("Please do not use Ctrl+C to Shutdown PnO! (attempting to die gracefully...)")
