...
def parse_cli_args():
    parser = argparse.ArgumentParser(
        description="PnO - Discord Bot's launcher (V3)", allow_abbrev=False
    )
    ...
    parser.add_argument("--start", "-s", help="Starts PnO", action="store_true")
    parser.add_argument(
        "--auto-restart", help="Autorestarts PnO in case of issues", action="store_true"
    )
...
def main_menu(flags_to_pass):
    if IS_WINDOWS:
        os.system("TITLE PnO - Discord Bot V3 Launcher")
    clear_screen()
    while True:
        print(INTRO)
