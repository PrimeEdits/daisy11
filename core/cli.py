...
async def interactive_config(red, token_set, prefix_set, *, print_header=True):
    ...
    if print_header:
        print("PnO - Discord Bot | Configuration process\n")
...
def parse_cli_flags(args):
    parser = argparse.ArgumentParser(
        description="PnO - Discord Bot", usage="redbot <instance_name> [arguments]"
    )
    parser.add_argument("--version", "-V", action="store_true", help="Show PnO's current version")
    ...
    parser.add_argument(
        "--no-cogs", action="store_true", help="Starts PnO with no cogs loaded, only core"
    )
    parser.add_argument(
        "--load-cogs",
        type=str,
